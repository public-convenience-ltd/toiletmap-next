...
import { ADMIN_ROLE_ID, __adminRoleTestUtils } from '../../src/middleware/require-admin-role';
...
const buildEnv = (): Env => ({
    ...
    AUTH0_MANAGEMENT_CLIENT_ID: process.env.AUTH0_MANAGEMENT_CLIENT_ID ?? `test-management-client-${randomUUID()}`,
    AUTH0_MANAGEMENT_CLIENT_SECRET: process.env.AUTH0_MANAGEMENT_CLIENT_SECRET ?? 'test-management-secret',
...
const buildSessionCookie = (options?: SessionOptions, activeEnv: Env | undefined = env) => {
    ...
    const hasCustomPermissions =
        options ? Object.prototype.hasOwnProperty.call(options, 'permissions') : false;
...
describe('Admin Routes', () => {
    it('should redirect to Auth0 login page when accessing /admin/login', async () => {
        ...
        expect(location).toContain(`client_id=${encodeURIComponent(env.AUTH0_CLIENT_ID)}`);
    ...
    it('should show an access denied page when the user is authenticated without admin role', async () => {
        const res = await app.request('/admin', { headers: authenticatedHeaders({ permissions: [] }) }, env);
    ...
    it('should clear the session if Auth0 revokes the admin permission mid-session', async () => {
        ...
        const res = await app.request('/admin', { headers: authenticatedHeaders({ sub: revokedSub }) }, env);
    ...
    it('should render loo detail page using shared API handlers', async () => {
        ...
        const res = await app.request(`/admin/loos/${mockLoo.id}`, { headers: authenticatedHeaders() }, env);
        expect(res.status).toBe(200);
        const text = await res.text();
    ...
    it('should embed persisted values into the edit form config', async () => {
        ...
        const res = await app.request(`/admin/loos/${mockLoo.id}/edit`, { headers: authenticatedHeaders() }, env);
        expect(res.status).toBe(200);
        expect(getById).toHaveBeenCalledWith(mockLoo.id);

        const text = await res.text();
        const config = extractJsonConfig<{ defaults: Record<string, unknown>; api: { update: string }; looId: string }>(
            text,
            'loo-edit-config',
        );
    ...
    it('should render a helpful state when trying to edit a missing loo', async () => {
        ...
        const res = await app.request('/admin/loos/missing/edit', { headers: authenticatedHeaders() }, env);
        expect(res.status).toBe(200);
    ...
    it('should render user statistics at /admin/users/statistics when session cookies are present', async () => {
        ...
        const res = await app.request('/admin/users/statistics', { headers: authenticatedHeaders() }, env);
        expect(res.status).toBe(200);
        const text = await res.text();
    ...
    it('should render the user administration page even when management credentials are missing', async () => {
        ...
        __adminRoleTestUtils.clearManagementClientCache();
        vi.mocked(Auth0ManagementClient.fromEnv).mockReturnValue(null as unknown as Auth0ManagementClient);
        const appWithoutMgmt = createApp(envWithoutMgmt);
        const res = await appWithoutMgmt.request('/admin/users/admin', { headers: authenticatedHeaders(undefined, envWithoutMgmt) }, envWithoutMgmt);
        expect(res.status).toBe(200);
        const text = await res.text();
    ...
    it('should grant permissions via the Auth0 management client', async () => {
        ...
        vi.spyOn(Auth0ManagementClient, 'fromEnv').mockReturnValue(mockClient as unknown as Auth0ManagementClient);
