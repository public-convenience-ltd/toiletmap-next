...
export type AreaFixtureOverrides = {
  id?: string;
  name?: string | null;
  type?: string | null;
...
let defaultFactory: FixtureFactory | null = null;
const getDefaultFactory = () => {
  if (!defaultFactory) {
    defaultFactory = new FixtureFactory();
  }
...
export const createAreaFixture = (overrides?: AreaFixtureOverrides) =>
  getDefaultFactory().areas.create(overrides);

export const createLooFixture = (overrides?: LooFixtureOverrides) =>
  getDefaultFactory().loos.create(overrides);

export const upsertLooFixture = (
  id: string,
  data: LooMutationAttributes,
  contributor: string,
) => getDefaultFactory().loos.upsert(id, data, contributor);

export const getLooById = (id: string) =>
  getDefaultFactory().loos.getRawById(id);
