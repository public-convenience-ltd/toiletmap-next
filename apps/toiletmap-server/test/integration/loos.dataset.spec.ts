...
import { type LegacyToiletRecord, legacyRecordToMutation } from "./utils/legacy-records";
import { callApi, jsonRequest } from "./utils/test-client";

const datasetPath = new URL("../../toilets-2025-11-04.json", import.meta.url);
const toiletsDataset = JSON.parse(readFileSync(datasetPath, "utf-8")) as LegacyToiletRecord[];

describe("Legacy dataset upsert coverage", () => {
  it("successfully parses the entire dataset with the Zod mutation schema", () => {
  ...
  it("upserts representative records via the API without validation errors", async () => {
    const sampleIndexes = [0, Math.floor(toiletsDataset.length / 2), toiletsDataset.length - 1];
