...
const authedJson = (method: string, body: unknown) => jsonRequest(method, body, authHeaders());
...
describe("Loo mutation endpoints", () => {
  describe("POST /api/loos", () => {
    ...
    it("validates custom ids are the correct length", async () => {
      ...
      const issueMessage = body.issues?.id?._errors?.[0] ?? body.issues?.id?._errors ?? "";
  ...
  describe("PUT /api/loos/:id", () => {
    ...
    it("updates an existing loo and returns 200", async () => {
      ...
      const response = await callApi(`/api/loos/${existing.id}`, authedJson("PUT", payload));
    ...
    it("validates request bodies", async () => {
      ...
      const response = await callApi(`/api/loos/${id}`, authedJson("PUT", invalidPayload));
      expect(response.status).toBe(400);
    });
  });
});
