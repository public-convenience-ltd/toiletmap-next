...
export const login = (c: Context<{ Bindings: Env }>) => {
...
export const callback = async (c: Context<{ Bindings: Env }>) => {
  ...
  try {
    ...
    let idTokenUser: Awaited<ReturnType<typeof authenticateToken>>;
    try {
      idTokenUser = await authenticateToken(tokenData.id_token, c.env, c.env.AUTH0_CLIENT_ID);
    } catch (error) {
    ...
    const userInfo = await fetchUserInfo(tokenData.access_token, c.env.AUTH0_ISSUER_BASE_URL);
