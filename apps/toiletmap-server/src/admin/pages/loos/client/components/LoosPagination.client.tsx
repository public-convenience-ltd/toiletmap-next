...
import type { SearchResponse, ScriptState } from '../utils/types';
...
export function LoosPagination({ searchData, state, currentPath }: LoosPaginationProps) {
    ...
    const paginationInfo =
        total === 0 ? 'No results to display' : `Showing ${startIndex}-${endIndex} of ${formatNumber(total)}`;
    ...
    const nextUrl = buildNavigationUrl(state, currentPath, { page: Math.min(totalPages, currentPage + 1) });
