...
import type { FilterMappings, ScriptState } from "./utils/types";
...
export function LoosListApp() {
  ...
  useEffect(() => {
    ...
    const queryConfig = isObject(parsedConfig.query)
      ? (parsedConfig.query as Record<string, unknown>)
      : {};
    const filtersInput = isObject(queryConfig.filters)
    ...
    const areaColors = Array.isArray(parsedConfig.areaColors)
      ? (parsedConfig.areaColors as unknown[]).filter(
          (color): color is string => typeof color === "string",
        )
      : [];
    const recentWindowDays = Number(parsedConfig.recentWindowDays) || 14;
    const currentPath =
      typeof parsedConfig.currentPath === "string"
        ? parsedConfig.currentPath
        : window.location.pathname;
    const apiConfig = isObject(parsedConfig.api)
      ? (parsedConfig.api as Record<string, unknown>)
      : {};
    const searchEndpoint =
      typeof apiConfig.search === "string" ? apiConfig.search : "/api/loos/search";
    const metricsEndpoint =
      typeof apiConfig.metrics === "string" ? apiConfig.metrics : "/api/loos/metrics";
