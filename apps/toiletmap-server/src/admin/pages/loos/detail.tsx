...
import { Badge, Button } from "../../components/DesignSystem";
...
const Section = (props: { title: string; children: unknown; className?: string }) => (
  <section class={`form-card ${props.className || ""}`} style="margin-bottom: var(--space-l);">
    <h2 class="section-eyebrow" style="margin-bottom: var(--space-m);">
      {props.title}
    </h2>
    {props.children}
  </section>
...
const Property = (props: { label: string; value: unknown; fullWidth?: boolean }) => (
  <div style={props.fullWidth ? "grid-column: 1 / -1;" : ""}>
    <dt class="form-label" style="font-size: var(--text-0); margin-bottom: var(--space-3xs);">
      {props.label}
    </dt>
    <dd style="margin: 0; font-size: var(--text-1); color: var(--color-neutral-dark-grey); word-break: break-word;">
      {props.value}
    </dd>
  </div>
);
...
const BooleanBadge = (props: { value: boolean | null; label?: string }) => {
  if (props.value === true) {
    return (
      <Badge variant="yes" icon="fa-check">
        {props.label || "Yes"}
      </Badge>
    );
  }
  if (props.value === false) {
    return (
      <Badge variant="no" icon="fa-xmark">
        {props.label || "No"}
      </Badge>
    );
  }
  return (
    <Badge variant="unknown" icon="fa-question">
      {props.label || "Unknown"}
    </Badge>
  );
};

const requestApiJson = async <T,>(
  ...
  const response = await looApiApp.fetch(request, c.env, executionCtx ?? fallbackExecutionCtx);
...
export const looDetail = async (c: Context<{ Bindings: Env }>) => {
  ...
  try {
    ...
    const reportsResult = await requestApiJson<ReportsApiResponse>(
      c,
      `/api/loos/${id}/reports?hydrate=true`,
    );
    ...
    return c.html(
      <Layout title="Error loading loo">
        ...
        <output class="notification notification--error">
          ...
          <div class="notification__content">
            ...
            <p class="notification__message">
              We could not retrieve the loo details. Please try again or return to the dataset.
              <br />
    ...
    <Layout title={`Loo ${loo.name || id}`}>
      ...
      <div class="page-header">
        ...
        <div style="display: flex; gap: var(--space-s); flex-wrap: wrap; justify-content: flex-start;">
          <Button href="/admin/loos" variant="secondary">
            Back
          </Button>
      ...
      {fromCreate && (
        <output class="notification notification--success" style="margin-bottom: var(--space-l);">
          <div class="notification__icon">
            <i class="fa-solid fa-circle-check" aria-hidden="true" />
          </div>
          <div class="notification__content">
            <p class="notification__title">New loo created</p>
            <p class="notification__message">The record was saved successfully.</p>
          </div>
        </output>
      )}
      ...
      {fromEdit && (
        <output class="notification notification--success" style="margin-bottom: var(--space-l);">
          ...
          <div class="notification__content">
            ...
            <p class="notification__message">Your changes have been saved.</p>
          </div>
        </output>
      )}
      ...
      <div class="loo-grid">
        <div>
          <Section title="Status & Access">
            <dl class="property-grid">
              <Property
                label="Active"
                value={
                  <BooleanBadge value={loo.active} label={loo.active ? "Active" : "Inactive"} />
                }
              />
              <Property
                label="Verified"
              ...
              <Property
                label="Fee"
                value={loo.noPayment ? "Free" : loo.paymentDetails || "Unknown"}
              />
            </dl>
          </Section>
          ...
          <Section title="Notes">
            <dl>
              <Property
                label="Public Notes"
                value={loo.notes || <span style="color: var(--color-neutral-grey);">No notes</span>}
                fullWidth
              />
              <br />
              <Property
                ...
                value={
                  loo.removalReason || <span style="color: var(--color-neutral-grey);">N/A</span>
                }
                fullWidth
              />
            </dl>
              ...
              {reports.map((report) => (
                <div class="timeline-item" key={report.id}>
                  ...
                  <div style="display: flex; justify-content: space-between; margin-bottom: var(--space-2xs);">
                    ...
                    <span style="color: var(--color-neutral-grey); font-size: var(--text-0);">
                  ...
                  <div style="font-size: var(--text-0);">
                    {report.diff ? (
                      <ul style="list-style: none; padding: 0; margin: 0;">
                        {Object.entries(report.diff).map(([key, value]) => {
                          const diffEntry = value as { previous: unknown; current: unknown };
                          return (
                            <li style="margin-bottom: var(--space-3xs);">
                              <span style="font-weight: 500; color: var(--color-neutral-dark-grey);">
                                {key}:
                              </span>{" "}
                              <span style="color: var(--color-accent-pink); text-decoration: line-through;">
                                {String(diffEntry.previous ?? "null")}
                              </span>{" "}
                              &rarr;{" "}
                              <span style="color: var(--color-accent-green);">
                                {String(diffEntry.current ?? "null")}
                              </span>
                            </li>
                          );
                        })}
                      ...
                      <span style="color: var(--color-neutral-grey);">
                        No changes recorded (Creation or legacy report)
                      </span>
                    )}
                  </div>
                </div>
        ...
        <div>
          {mapUrl && (
            <div
              class="form-card"
              style="margin-bottom: var(--space-l); padding: 0; overflow: hidden;"
            >
              <iframe
                width="100%"
                height="300"
          ...
          <div class="form-card">
            <h3 class="section-eyebrow" style="margin-bottom: var(--space-m);">
              Actions
            </h3>
            <div style="display: grid; gap: var(--space-s);">
              <Button href={`/api/loos/${loo.id}`} variant="secondary">
                View API Response
              </Button>
              <Button href={`/loos/${loo.id}`} variant="secondary">
                View Public Page
              </Button>
            </div>
          </div>
