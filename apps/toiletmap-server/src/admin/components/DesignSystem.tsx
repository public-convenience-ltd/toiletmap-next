export const Button = (props: {
  ...
  return (
    <button type={props.type || "button"} class={className} onclick={props.onClick}>
      {props.children}
    </button>
  );
};
...
export const Badge = (props: {
  ...
  return (
    <span class={className} title={props.title}>
      {props.icon && <i class={`fa-solid ${props.icon}`} aria-hidden="true" />}
...
export const CollapsibleCard = ({
  ...
  const classes = [
    ...
    "collapsible",
    className,
    defaultOpen ? "" : "collapsible--collapsed",
  ]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    <section class={classes} data-collapsible={id}>
      <div class="collapsible__header">
        <div>
          {eyebrow && (
            <p class="section-eyebrow" style="margin-bottom: var(--space-3xs);">
              {eyebrow}
            </p>
          )}
          <h2 class="section-title" style="margin-bottom: var(--space-3xs);">
            {title}
          </h2>
        ...
        >
          <i class="fa-solid fa-chevron-up collapsible__toggle-icon" aria-hidden="true" />
